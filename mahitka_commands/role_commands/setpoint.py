...
@bot.hybrid_command(name="setpoint", description="set the number of points for the user")
@app_commands.describe(value="number of points", user="user mention")
async def setpoint(ctx: commands.context.Context, value: int, user: discord.Member):
    ...
    if await basa.chek(member_id=user.id):
        ...
        await basa.set_point(memberId=user.id, value=value)
